...
path_ml = f'{paths.npy}/ml30yr_masked.npy'
...
alt_max = 0.121
data_fj = data_fj[:, (data_fj[3] < alt_max)] # 3 is hybrid height.
data_ml = data_ml[:, (data_ml[3] < alt_max)]
...
data_fj = data_fj[:, idx_fj]
data_ml = data_ml[:, idx_ml]
'''

# Problematic items to test.
problems = [12, 13, 15, 16] # 17, 22, 25, 29, 36, 38, 41, 42]
for i in problems:
  item_fj, item_ml = data_fj[i], data_ml[i] 
  item_name = names[i] 
  
  # Spatial and temporal features.
  for j in range(1):
    feature_fj, feature_ml = data_fj[j], data_ml[j] 
    ...
    # Make plot.
    fig, ax = plt.subplots()
    ax.plot(features_fj, item_avg_fj, label=f'UM with Fast-JX photolysis.')
    ax.plot(features_ml, item_avg_ml, label=f'UM with random forest photolysis.')
    ax.set_title(f'Median {item_name[0]} {item_name[1]} by {feature_name[0]} in a 30-year simulation')
  ...
  plt.title(f'Targets and predictions of tropospheric {item_name[0]} {item_name[1]}')
